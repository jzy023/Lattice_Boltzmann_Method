...
void confinedBox::equilibrium()
{
    ...
    for (int i = 0; i < ny_; i++)
    {
        for (int j = 0; j < nx_; j++)
        {
            ...
            feq_[i][j][NE] = (1./36.) * r * (1 + 3 * uxy + 4.5 * uxySqr - 1.5 * usqr_[i][j]);
            feq_[i][j][NW] = (1./36.) * r * (1 + 3 * (uy_[i][j] - ux_[i][j]) + 4.5 * (uy_[i][j] - ux_[i][j]) * (uy_[i][j] - ux_[i][j]) - 1.5 * usqr_[i][j]);
            feq_[i][j][SW] = (1./36.) * r * (1 + 3 * (-uxy) + 4.5 * uxySqr - 1.5 * usqr_[i][j]);
...
void confinedBox::boundaryConditions()
{
    // #pragma omp parallel for
    for (int i = 0; i < ny_; i++)
    {
    ...
    {   
        ...
        f_[0][j][NW] = f_[0][j][SE];
        // Top wall
        f_[ny_-1][j][S] = f_[ny_-1][j][N];
        f_[ny_-1][j][SW] = f_[ny_-1][j][NE];
        f_[ny_-1][j][SE] = f_[ny_-1][j][NW];
    }
}


void confinedBox::collideStream()
{
    // Colliding
...
// methods
void confinedBox::macroscopic()
{
    umagMax_ = 0; //0.98*umagMax_;
...
void confinedBox::mkdirDataOutput
...
void confinedBox::wrtieDataCSV(int sampleFreq)
{
